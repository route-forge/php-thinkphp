...
- `src/Console/*` — `route:forge:list|types|clear`；`execute` 覆写为 `$this->app->invoke([$this,'handle'])` 吃容器方法注入；命令内先 `event->trigger(RouteLoaded::class)` 加载路由文件（同 think 自带 `route:list`）。
